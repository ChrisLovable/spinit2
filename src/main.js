...
// Hide countdown timer by default - only show 10 mins after last ticket sold
...
if (savedAutoSpin) {
  ...
  if (!isNaN(dt.getTime()) && dt > new Date()) {
    ...
    // Show countdown ONLY if auto-spin is scheduled (10 mins after last ticket)
    if (countdownTimer) {
...
async function loadActiveCompetitions() {
  try {
    ...
    // Filter to only show competitions that have NOT sold all tickets
    console.log(`📋 Filtering ${competitions.length} competitions to find active ones (not all tickets sold)...`);
    ...
    console.log(`📋 Found ${activeCompetitions.length} active competitions (out of ${competitions.length} total)`);
    ...
    if (activeCompetitions.length === 0) {
      ...
      console.log('⚠️ No active competitions found (all have sold all tickets)');
    } else {
...
// Logo spinning animation - spins every 30 seconds using same logic as wheel
...
function spinLogo() {
  ...
  isLogoSpinning = true;
  const duration = 20000; // 20 seconds, same as wheel
  const startTime = Date.now();
  const startRotation = 0;
  
  // Calculate full rotations (5-8 rotations like the wheel)
  const fullRotations = 5 + Math.random() * 3; // 5-8 rotations
  const targetRotation = fullRotations * 360; // Always complete full rotations to land right side up
  
  // Easing function similar to wheel (ease-out)
  ...
  function animate() {
    ...
    logo.style.transition = 'none'; // Disable CSS transitions during animation
    
    if (progress < 1) {
      requestAnimationFrame(animate);
    } else {
      // Animation complete - ensure it's right side up (0 degrees)
      logo.style.transform = 'rotate(0deg)';
      logo.style.transition = 'transform 0.3s ease'; // Smooth final transition
...
// Start logo spinning every 30 seconds
function startLogoSpinInterval() {
  // Clear any existing interval
  if (logoSpinInterval) {
    clearInterval(logoSpinInterval);
  }
  
  // Spin immediately on page load
  setTimeout(spinLogo, 1000);
  
  // Then spin every 30 seconds
  logoSpinInterval = setInterval(spinLogo, 30000);
}

// Initialize logo spinning when page loads
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startLogoSpinInterval);
} else {
  startLogoSpinInterval();
}
